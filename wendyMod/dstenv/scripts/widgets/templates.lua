local AccountItemFrame = require"widgets/accountitemframe"
local Grid = require"widgets/dstgrid"
...
TEMPLATES.old = require"widgets/templates2"
...
local function ReduxBackground(variation)
    ...
    bg.bgplate = bg:AddChild(TEMPLATES._CreateBackgroundPlate(
        Image("images/bg_redux_" .. variation .. ".xml", variation .. ".tex")))
...
function TEMPLATES.BoarriorBackground() return ReduxBackground("labg") end

function TEMPLATES.BrightMenuBackground() return ReduxBackground("dark_right") end
...
function TEMPLATES.BackgroundTint(a, rgb) return TEMPLATES.old.BackgroundTint(a, rgb) end
...
function TEMPLATES.ClayWargBackground()
    ...
    anim_bg.OnUpdate = function(self, dt)
        ...
        if self.fadet > 96 * FRAMES then self.fadet = self.fadet - 96 * FRAMES end
...
function TEMPLATES.GetBuildString()
    local version_str = BRANCH == "dev" and "Internal" or BRANCH == "staging" and "Preview" or
                            STRINGS.UI.MAINSCREEN.DST_UPDATENAME

    return version_str .. " v" .. APP_VERSION .. " (" ..
               (APP_ARCHITECTURE == "x32" and "32-bit" or APP_ARCHITECTURE == "x64" and "64-bit" or "??-bit") .. ")"
end
...
function TEMPLATES.AddBuildString(parent_widget, config)
    ...
    if config.align ~= nil then version:SetHAlign(config.align) end
    if config.w ~= nil and config.h ~= nil then version:SetRegionSize(config.w, config.h) end
...
function TEMPLATES.MenuButton(text, onclick, tooltip_text, tooltip_widget, style, text_size)
    ...
    local btn = ImageButton("images/dst_global_redux.xml", "blank.tex", -- never used, hidden
    nil, nil, nil, "menu_selected.tex", image_scale, image_offset)
    ...
    btn.bg = btn:AddChild(Image("images/dstui.xml", "blank.tex"))
    local w, h = btn.text:GetRegionSize()
    btn.bg:ScaleToSize(text_region_width, h + 15)
    btn.bg:SetPosition(-10 + text_offset_x, 1)

    btn.ongainfocus = function(is_enabled) if tooltip_widget ~= nil then tooltip_widget:SetString(tooltip_text) end end

    btn.onlosefocus = function()
        if btn.parent and not btn.parent.focus and tooltip_widget ~= nil then tooltip_widget:SetString("") end
    end
...
function TEMPLATES.TwoLineMenuButton(text, onclick, tooltip_text, tooltip_widget)
    ...
    btn:SetTextures("images/frontend_redux.xml", "menu_wardrobe_selection.tex", -- never used, hidden
    nil, nil, nil, "menu_wardrobe_selection.tex", {0.6}, {-10, 1})
    ...
    btn.SetSecondaryText = function(self, second_text) self.secondary_text:SetString(second_text or "") end
...
function TEMPLATES.WardrobeButtonMinimal(onclick)
    local btn = ImageButton("images/dst_global_redux.xml", "blank.tex", -- never used, hidden
    nil, nil, nil, "blank.tex", {0.6}, {-10, 1})
    btn.scale_on_focus = false
    btn:SetImageNormalColour(1, 1, 1, 0) -- we don't want anything shown for normal.
    btn:SetImageFocusColour(1, 1, 1, 0) -- use focus overlay instead.
    btn:SetImageSelectedColour(1, 1, 1, 1)

    btn.ongainfocus = function(is_enabled) btn.icon:ShowFocus(true) end
    btn.onlosefocus = function() btn.icon:ShowFocus(false) end
    btn.onselect = function() btn.icon:ShowSelect(true) end
    btn.onunselect = function() btn.icon:ShowSelect(false) end
    btn:SetOnClick(onclick)
    ...
    btn.SetItem = function(self, item_id) self.icon:SetItem(item_id) end
...
function TEMPLATES.BackButton(onclick, txt, shadow_offset, scale)
    local btn = ImageButton("images/frontend.xml", "turnarrow_icon.tex", "turnarrow_icon_over.tex", nil, nil, nil,
        {1, 1}, {0, 0})
    ...
    btn.bg = btn.text:AddChild(Image("images/dstui.xml", "blank.tex"))
    ...
    btn.SetText = function(btn_inst, msg, dropShadow, dropShadowOffset)
        ...
        local text_x = w / 2 + 30
        ConfigureText(btn.text, text_x, {
            x = 0,
            y = 0
        })
        ConfigureText(btn.text_shadow, text_x, shadow_offset or {
            x = 2,
            y = -1
        })
    end
    ...
    btn:SetOnGainFocus(function() btn:SetScale(btn.scale + .05) end)
    btn:SetOnLoseFocus(function() btn:SetScale(btn.scale) end)
...
function TEMPLATES.StandardButton(onclick, txt, size, icon_data)
    ...
    local btn = ImageButton("images/dst_global_redux.xml", prefix .. "_normal.tex", prefix .. "_hover.tex",
        prefix .. "_disabled.tex", prefix .. "_down.tex")
    btn:SetOnClick(onclick)
    btn:SetText(txt)
...
function TEMPLATES.IconButton(iconAtlas, iconTexture, labelText, sideLabel, alwaysShowLabel, onclick, textinfo,
    defaultTexture)
    ...
    if sideLabel then
        ...
        btn.label = btn:AddChild(Text(textinfo.font or NEWFONT, textinfo.size or 25, labelText,
            textinfo.colour or UICOLOURS.GOLD_CLICKABLE))
...
function TEMPLATES.StandardCheckbox(onclick, size, init_checked, helptext, hovertext_info)
    ...
    local function SetChecked(checked)
        if checked then
            checkbox:SetTextures("images/dst_global_redux.xml", "checkbox_normal_check.tex", "checkbox_focus_check.tex",
                "checkbox_normal.tex", nil, nil, {1, 1}, {0, 0})
        else
            checkbox:SetTextures("images/dst_global_redux.xml", "checkbox_normal.tex", "checkbox_focus.tex",
                "checkbox_normal_check.tex", nil, nil, {1, 1}, {0, 0})
        end
    end
    ...
    if helptext ~= nil then checkbox:SetHelpTextMessage(helptext) end
...
function TEMPLATES.ListItemBackground(row_width, row_height, onclick_fn)
    ...
    local row = ImageButton("images/frontend_redux.xml", prefix .. "_normal.tex", -- normal
    nil, -- focus
    nil, nil, prefix .. "_selected.tex" -- selected
    )
...
function TEMPLATES.ModListItem(onclick_btn, onclick_checkbox, onclick_setfavorite)
    ...
    opt.SetModEnabled = function(_, should_enable)
        if should_enable then
            ...
            opt.checkbox:SetTextures("images/dst_global_redux.xml", "checkbox_normal_check.tex", "checkbox_focus_check.tex",
                "checkbox_normal.tex", nil, nil, {1, 1}, {0, 0})
        else
            opt.image:SetTint(unpack(opt.image_disabled_tint))
            opt.checkbox:SetTextures("images/dst_global_redux.xml", "checkbox_normal.tex", "checkbox_focus.tex",
                "checkbox_normal_check.tex", nil, nil, {1, 1}, {0, 0})
        end
    end
    ...
    opt.SetModFavorited = function(_, should_favorite)
        if should_favorite then
            opt.setfavorite:SetTextures("images/dst_global_redux.xml", "star_checked.tex", nil, "star_uncheck.tex", nil,
                nil, {0.75, 0.75}, {0, 0})
        else
            opt.setfavorite:SetTextures("images/dst_global_redux.xml", "star_uncheck.tex", nil, "star_checked.tex", nil,
                nil, {0.75, 0.75}, {0, 0})
        end
    end
    ...
    opt.SetMod = function(_, modname, modinfo, modstatus, isenabled, isfavorited)
        if modinfo and modinfo.icon_atlas and modinfo.icon then
            ...
            opt.image:SetTexture("images/dstui.xml", "portrait_bg.tex")
        end
...
function TEMPLATES.ModListItem_Downloading()
    ...
    opt.SetMod = function(_, mod)
        opt.name:SetString(subfmt(STRINGS.UI.MODSSCREEN.DOWNLOADINGMOD, {
            name = mod.fancy_name
        }))
    end

    opt.Select = function(_) end

    opt.Unselect = function(_) end
...
function TEMPLATES.DoodadCounter(number_of_doodads)
    ...
    doodad._CountFn = function(self)
        if self.num_display_doodads ~= self.num_doodads then
            ...
            self.inst:DoTaskInTime(FRAMES, function() self:_CountFn() end)
        end
...
function TEMPLATES.KleiPointsCounter(number_of_points)
    ...
    points:SetOnClick(function() TheFrontEnd:GetAccountManager():VisitAccountPage("rewards") end)
...
function TEMPLATES.StandardSingleLineTextEntry(fieldtext, width_field, height, font, font_size, prompt_text)
    ...
    wdg.textbox_bg = wdg:AddChild(Image("images/dst_global_redux.xml", "textbox3_gold_normal.tex"))
    wdg.textbox_bg:ScaleToSize(width_field, height)
    wdg.textbox = wdg:AddChild(TextEdit(font or CHATFONT, (font_size or 25) * textbox_font_ratio, fieldtext,
        UICOLOURS.BLACK))
    -- wdg.textbox:SetForceEdit(true)--#FIXME
    ...
    wdg.textbox:SetFocusedImage(wdg.textbox_bg, "images/dst_global_redux.xml", "textbox3_gold_normal.tex",
        "textbox3_gold_hover.tex", "textbox3_gold_focus.tex")

    if prompt_text then wdg.textbox:SetTextPrompt(prompt_text, UICOLOURS.GREY) end
    ...
    wdg.GetHelpText = function(self)
        ...
        if not self.textbox.editing and not self.textbox.focus then
            table.insert(t, TheInput:GetLocalizedControl(controller_id, CONTROL_ACCEPT, false, false) .. " " ..
                STRINGS.UI.HELP.CHANGE_TEXT)
        end
...
function TEMPLATES.LabelTextbox(labeltext, fieldtext, width_label, width_field, height, spacing, font, font_size,
    horiz_offset)
...
function TEMPLATES.LabelSpinner(labeltext, spinnerdata, width_label, width_spinner, height, spacing, font, font_size,
    horiz_offset, onchanged_fn, colour, tooltip_text)
...
function TEMPLATES.LabelNumericSpinner(labeltext, min, max, width_label, width_spinner, height, spacing, font,
    font_size, horiz_offset, tooltip_text)
    width_label = width_label or 220
    width_spinner = width_spinner or 150
    height = height or 40
...
function TEMPLATES.LabelButton(onclick, labeltext, buttontext, width_label, width_button, height, spacing, font,
    font_size, horiz_offset)
    local offset = horiz_offset or 0
...
function TEMPLATES.OptionsLabelCheckbox(onclick, labeltext, checked, width_label, width_button, height, checkbox_size,
    spacing, font, font_size, horiz_offset, tooltip_text)
    local offset = horiz_offset or 0
    local total_width = width_label + width_button + spacing
    local wdg = Widget("labelbutton")
    wdg.label = wdg:AddChild(Text(font or NEWFONT, font_size or 25, labeltext))
...
function TEMPLATES.LabelCheckbox(onclick, checked, text)
    ...
    checkbox.Refresh = function(self)
        if self.checked then
            self:SetTextures("images/dst_global_redux.xml", "checkbox_normal_check.tex", "checkbox_focus_check.tex",
                "checkbox_normal_check.tex", nil, nil, {1, 1}, {0, 0})
        else
            self:SetTextures("images/dst_global_redux.xml", "checkbox_normal.tex", "checkbox_focus.tex",
                "checkbox_normal.tex", nil, nil, {1, 1}, {0, 0})
        end
    end
...
function TEMPLATES.StandardSpinner(spinnerdata, width_spinner, height, font, font_size, onchanged_fn, colour)
    local atlas = "images/dst_global_redux.xml"
    local lean = true
    local wdg = Spinner(spinnerdata, width_spinner, height, {
        font = font or CHATFONT,
        size = font_size or 25
    }, nil, atlas, nil, lean)
...
function TEMPLATES.StandardNumericSpinner(min, max, width_spinner, height, font, font_size)
    local atlas = "images/dst_global_redux.xml"
    local lean = true
    local wdg = NumericSpinner(min, max, width_spinner, height, {
        font = font or CHATFONT,
        size = font_size or 25
    }, atlas, nil, nil, lean)
...
function TEMPLATES.ChatFlairBadge()
    ...
    flair.SetFestivalBackground = function(self, festival_key)
        ...
        self.bg:SetTexture("images/profileflair.xml", "playericon_bg_" .. festival_key .. ".tex",
            "playericon_bg_none.tex")
    end
    ...
    flair.SetFlair = function(self, profileflair)
        ...
        if self.profileflair then
            ...
            if profileflair == "default" then profileflair = nil end
            self.flair_img:SetTexture(GetProfileFlairAtlasAndTex(profileflair))
        end
    end

    flair.GetFlair = function(self) return self.profileflair end
    ...
    flair.GetSize = function(self) return self.flair_img:GetScaledSize() end
...
function TEMPLATES.AnnouncementBadge()
    ...
    announcement.SetAnnouncement = function(self, announcement)
        ...
        if announcement then
            ...
            self.announcement_img:SetTexture(icon_info.atlas or "images/button_icons.xml",
                icon_info.texture or "announcement.tex")
        end
    end

    announcement.GetAnnouncement = function(self) return self.announcement end
    ...
    announcement.GetSize = function(self) return self.announcement_img:GetScaledSize() end
...
function TEMPLATES.SystemMessageBadge()
    ...
    systemmessage.GetSize = function(self) return self.systemmessage_img:GetScaledSize() end
...
function TEMPLATES.RankBadge()
    ...
    rank.SetFestivalBackground = function(self, festival_key)
        festival_key = festival_key or "none"
        rank.bg:SetTexture("images/profileflair.xml", "playericon_bg_" .. festival_key .. ".tex",
            "playericon_bg_none.tex")
    end
    -- Assume current event.
    ...
    rank.SetRank = function(self, profileflair, rank_value, hide_hover_text)
        if not IsAnyFestivalEventActive() then rank_value = nil end

        if not hide_hover_text and IsItemId(profileflair) then
            rank.flair:SetHoverText(GetSkinName(profileflair), {
                font = UIFONT,
                offset_x = 0,
                offset_y = 40,
                colour = GetColorForItem(profileflair)
            })
        else
...
function TEMPLATES.UserProgress(onclick)
    ...
    progress.btn = progress:AddChild(ImageButton("images/dstui.xml", "blank.tex"))
...
function TEMPLATES.LargeScissorProgressBar(name)
    ...
    local frame = bar:AddChild(Image("images/dst_global_redux.xml", "progressbar_wxplarge_frame.tex"))
    frame:SetPosition(-2, 0)

    local fill = bar:AddChild(Image("images/dst_global_redux.xml", "progressbar_wxplarge_fill.tex"))
...
function TEMPLATES.WxpBar()
    ...
    local xp_cur_title = wxpbar:AddChild(Text(HEADERFONT, 16, STRINGS.UI.WXPLOBBYPANEL.WXP_CURRENT_XP,
        UICOLOURS.HIGHLIGHT_GOLD))
    xp_cur_title:SetPosition(-190, -33)
    xp_cur_title:SetHAlign(ANCHOR_LEFT)
    xp_cur_title:SetRegionSize(200, font_size + 5)

    local xp_next_title = wxpbar:AddChild(Text(HEADERFONT, 16, STRINGS.UI.WXPLOBBYPANEL.WXP_NEXT_LEVEL_XP,
        UICOLOURS.HIGHLIGHT_GOLD))
...
function TEMPLATES.ItemImageText(item_type, item_key, max_width)
    ...
    local w = TEMPLATES.old.ItemImageText(item_type, item_key, iconScale, font, textsize, label, colour, textwidth,
        image_offset)
...
function TEMPLATES.CurlyWindow(sizeX, sizeY, title_text, bottom_buttons, button_spacing, body_text)
    ...
    if title_text then
        ...
        if JapaneseOnPS4() then w.title:SetSize(40) end
    end
    ...
    if bottom_buttons then
        ...
        if is_tight_bottom_fit then button_height = 60 end
    ...
    if body_text then
        ...
        if bottom_buttons then height_reduction = 30 end
...
function TEMPLATES.RectangleWindow(sizeX, sizeY, title_text, bottom_buttons, button_spacing, body_text)
    ...
    w.SetBackgroundTint = function(self, r, g, b, a)
        for i = 4, 5 do self.elements[i]:SetTint(r, g, b, a) end
        self.mid_center:SetTint(r, g, b, a)
    end

    w.HideBackground = function(self)
        for i = 4, 5 do self.elements[i]:Hide() end
        self.mid_center:Hide()
    end

    w.InsertWidget = function(self, widget)
        w:AddChild(widget)
        for i = 1, 3 do self.elements[i]:MoveToFront() end
        for i = 6, 8 do self.elements[i]:MoveToFront() end
        w.bottom:MoveToFront()
        return widget
    end
...
function TEMPLATES.ControllerFunctionsFromButtons(buttons)
    if not buttons or not next(buttons) then return function() return false end, function() return "" end end
    ...
    if not has_controls_specified then
        ...
        if #buttons > 1 and last_button then last_button.controller_control = CONTROL_CANCEL end
    end
...
function TEMPLATES.ScrollingGrid(items, opts)
    ...
    local function ScrollWidgetsCtor(context, parent, scroll_list)
        ...
        parent.grid:SetPosition(-opts.widget_width * (opts.num_columns - 1) / 2,
            opts.widget_height * (opts.num_visible_rows - 1) / 2 + peek_height / 2)
        ...
        for i, w in ipairs(widgets) do w:MoveToBack() end
        ...
        if opts.allow_bottom_empty_row then end_offset = 1 end
    ...
    local scroller = TrueScrollList(opts.scroll_context, ScrollWidgetsCtor, opts.apply_fn, scissor_x, scissor_y,
        scissor_width, scissor_height, opts.scrollbar_offset, opts.scrollbar_height_offset, opts.scroll_per_click)
    scroller:SetItemsData(items)
    scroller.GetScrollRegionSize = function(self) return scissor_width, scissor_height end
    return scroller
end
...
function TEMPLATES.MakeUIStatusBadge(_status_name, c)
    ...
    status.status_icon:SetTexture("images/dst_global_redux.xml", "status_" .. _status_name .. ".tex")
    status.status_icon:SetScale(.55)

    status.status_image = status:AddChild(Image("images/dst_global_redux.xml", "value_gold.tex"))
    ...
    status.ChangeCharacter = function(self, character)
        ...
        status.status_icon:SetTexture("images/dst_global_redux.xml", "status_" .. status_name .. ".tex")

        local v = tostring(TUNING[string.upper(character .. "_" .. status_name)] or
                               --STRINGS.CHARACTER_DETAILS.STAT_UNKNOW)--#FIXME
                               "unknown")
        status.status_value:SetString(v)
    end

    if c ~= nil then status:ChangeCharacter(c) end
...
function TEMPLATES.MakeStartingInventoryWidget(c, left_align)
    ...
    local title = root:AddChild(Text(HEADERFONT, 25, --STRINGS.CHARACTER_DETAILS.STARTING_ITEMS_TITLE,--#FIXME
    "starting items",
        UICOLOURS.GOLD_UNIMPORTANT))
    ...
    root.ChangeCharacter = function(self, character)
        ...
        local inv_item_list = (TUNING.GAMEMODE_STARTING_ITEMS[TheNet:GetServerGameMode()] or
                                  TUNING.GAMEMODE_STARTING_ITEMS.DEFAULT)[character]
        if inv_item_list ~= nil and #inv_item_list > 0 then
            ...
            for i, item in ipairs(inv_items) do
                ...
                local atlas = override_item_image ~= nil and override_item_image.atlas or
                                  GetInventoryItemAtlas(item .. ".tex", true)
                if atlas ~= nil then
            ...
            -- no gear
            local label = root._invitems:AddChild(
                Text(HEADERFONT, 21, -- STRINGS.CHARACTER_DETAILS.STARTING_ITEMS_NONE,--#FIXME
                "none", UICOLOURS.GREY))
    ...
    if c ~= nil then root:ChangeCharacter(c) end
