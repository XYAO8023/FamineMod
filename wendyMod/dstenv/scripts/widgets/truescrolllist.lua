...
-- region.
local TrueScrollList = Class(Widget,
    function(self, context, create_widgets_fn, update_fn, scissor_x, scissor_y, scissor_width, scissor_height,
        scrollbar_offset, scrollbar_height_offset, scroll_per_click)
        Widget._ctor(self, "TrueScrollList")
        _G.ts=self
        assert(create_widgets_fn ~= nil and update_fn ~= nil,
            "TrueScrollList requires create widgets and update functions")
        ...
        self.bg = self:AddChild(Image("images/dstui.xml", "blank.tex"))
        ...
        self.widgets_to_update, self.widgets_per_row, self.row_height, self.visible_rows, self.end_offset =
            create_widgets_fn(self.context, self.list_root, self)
        ...
        self.scrollbar_offset = {scissor_x + scissor_width + (scrollbar_offset or 0), scissor_y + scissor_height / 2}
...
function TrueScrollList:DebugDraw_AddSection(dbui, panel)
    ...
    if self.scissor_preview.image == nil then
        self.scissor_preview.image = self.scissored_root:AddChild(Image("images/dstui.xml", "white.tex"))
    ...
    do
        ...
        local changed_pos, x, y = dbui.DragFloat3("scissor pos", self.scissor_preview.x, self.scissor_preview.y, 0, 1,
            -900, 900)
        if changed_pos then self.scissor_preview.x, self.scissor_preview.y = x, y end
        local changed_size, w, h = dbui.DragFloat3("scissor size", self.scissor_preview.width,
            self.scissor_preview.height, 0, 1, 10, 900)
        ...
        if force_reposition or changed_pos then
            ...
            self.scissor_preview.image:SetPosition(self.scissor_preview.x + self.scissor_preview.width / 2,
...
function TrueScrollList:BuildScrollBar()
    ...
    self.up_button =
        self.scroll_bar_container:AddChild(ImageButton("images/dst_global_redux.xml", "scrollbar_arrow_up.tex"))
    ...
    self.up_button:SetOnClick(function() self.last_up_button_time = nil end)

    self.down_button = self.scroll_bar_container:AddChild(ImageButton("images/dst_global_redux.xml",
        "scrollbar_arrow_down.tex"))
    ...
    self.down_button:SetOnClick(function() self.last_down_button_time = nil end)
    ...
    self.scroll_bar_line = self.scroll_bar_container:AddChild(Image("images/dst_global_redux.xml", "scrollbar_bar.tex"))
    ...
    self.scroll_bar = self.scroll_bar_container:AddChild(ImageButton("images/dstui.xml", "1percent_clickbox.tex",
    ...
    self.position_marker = self.scroll_bar_container:AddChild(
        ImageButton("images/dst_global_redux.xml", "scrollbar_handle.tex"))
    ...
    self.position_marker:SetWhileDown(function() self:DoDragScroll() end)
...
function TrueScrollList:GetListWidgets() return self.widgets_to_update end
...
function TrueScrollList:OnUpdate(dt)
    if self.control_scroll_repeat_time ~= nil then
        -- Scroll repeat
        if not (TheInput:IsControlPressed(self.control_up) or TheInput:IsControlPressed(self.control_down)) then
        ...
        elseif TheInput:IsControlPressed(self.control_up) then
            local repeat_time = TheInput:GetControlIsMouseWheel(self.control_up) and MOUSE_SCROLL_REPEAT_TIME or
                                    SCROLL_REPEAT_TIME
            ...
            local repeat_time = TheInput:GetControlIsMouseWheel(self.control_down) and MOUSE_SCROLL_REPEAT_TIME or
                                    SCROLL_REPEAT_TIME
            if self.control_scroll_repeat_time < 0 then
                self.control_scroll_repeat_time = repeat_time > dt and repeat_time - dt or 0
            else
    ...
    self.current_scroll_pos = math.abs(self.current_scroll_pos - self.target_scroll_pos) > 0.01 and
                                  Lerp(self.current_scroll_pos, self.target_scroll_pos, 0.25) or self.target_scroll_pos
...
function TrueScrollList:Scroll(scroll_step) self.target_scroll_pos = self.target_scroll_pos + scroll_step end
...
function TrueScrollList:FindDataIndex(target_data)
    if target_data ~= nil then for i = 1, #self.items do if self.items[i] == target_data then return i end end end
    return nil
end
...
function TrueScrollList:CanScroll() return self.end_pos > 1 end

function TrueScrollList:GetPositionScale() return (self.current_scroll_pos - 1) / (self.end_pos - 1) end

function TrueScrollList:GetSlideStart() return self.scrollbar_height / 2 - arrow_button_size end

function TrueScrollList:GetSlideRange() return self.scrollbar_height - 2 * arrow_button_size end
...
function TrueScrollList:RefreshView()
    ...
    -- TheFrontEnd:DoHoverFocusUpdate(true)--#FIXME:deleted temporary
end
...
function TrueScrollList:GetNextWidget(dir)
    ...
    local last_fully_visible_index = first_fully_visible_index +
                                         (math.floor((self.scissor_height - used_row_height) / self.row_height) *
                                             self.widgets_per_row)
...
function TrueScrollList:OnFocusMove(dir, down)
    ...
    if dir == MOVE_UP or dir == MOVE_DOWN then if self:GetNextWidget(dir) then return true end end
...
function TrueScrollList:OverrideControllerButtons(control_up, control_down, hints_enabled)
    ...
    if hints_enabled then
        self.up_button_controllerhint:SetString(
            TheInput:GetLocalizedControl(TheInput:GetControllerID(), self.control_up))
        self.down_button_controllerhint:SetString(TheInput:GetLocalizedControl(TheInput:GetControllerID(),
            self.control_down))
...
function TrueScrollList:OnControl(control, down)
    ...
    if down and (self.focus or FunctionOrValue(self.custom_focus_check)) and self.scroll_bar:IsVisible() then
        if control == self.control_up then
            ...
            if TheInput:ControllerAttached() then scroll_amt = scroll_amt / 2 end
...
function TrueScrollList:GetHelpText()
    ...
    local t = {}
    if self:CanScroll() then
        table.insert(t, TheInput:GetLocalizedControl(controller_id, self.control_up) .. "/" ..
            TheInput:GetLocalizedControl(controller_id, self.control_down) .. " " .. STRINGS.UI.HELP.SCROLL)
    end
