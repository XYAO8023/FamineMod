...
local _Badge = require "widgets/badge"

local Badge = Class(_Badge,
    function(self, anim, owner, tint, iconbuild, circular_meter, use_clear_bg, dont_update_while_paused)
        --[[
            -- self:UpdateWhilePaused(not dont_update_while_paused)--#TODO: patch these things
        ]]
        ...
        if anim ~= nil then
            _Badge._ctor(self, anim or "nil", owner, tint, iconbuild, circular_meter, use_clear_bg,
            ...
            self.backing:MoveToBack()
            self.pulse:MoveToBack() -- fix layer misplacement

            -- self.anim = self:AddChild(UIAnim())
            self.anim:GetAnimState():SetBank("status_meter")
            self.anim:GetAnimState():SetBuild("status_meter")
            self.anim:GetAnimState():PlayAnimation("anim")
            if tint ~= nil then self.anim:GetAnimState():SetMultColour(unpack(tint)) end
            ...
            if circular_meter then
                ...
                if tint ~= nil then self.circular_meter:GetAnimState():SetMultColour(unpack(tint)) end
        ...
        -- added compatibility with Combined Status
...
function Badge:SetIconSkin(skinname)
    if self.iconbuild ~= nil then -- Do we want to allow a skin on the icon for badges that didn't have a default?
        if skinname and skinname ~= "" then -- patched for nil skinname, should remove when skin api is ready
...
function Badge:OnGainFocus()
    _Badge.OnGainFocus(self)
    self.num:Show()
end

function Badge:OnLoseFocus()
    _Badge.OnLoseFocus(self)
    self.num:Hide()
end

function Badge:SetPercent(val, max)
    _Badge.SetPercent(self, val, max)
...
function Badge:PulseGreen()
    ...
    if self.warning.shown then self.warning:Hide() end

    if self.warningdelaytask ~= nil then self.warningdelaytask:Cancel() end
    self.warningdelaytask = self.inst:DoTaskInTime(2 * self.pulse:GetAnimState():GetCurrentAnimationLength(),
        CheckWarning, self)
end
...
function Badge:PulseRed()
    ...
    if self.warningdelaytask ~= nil then self.warningdelaytask:Cancel() end
    self.warningdelaytask = self.inst:DoTaskInTime(self.pulse:GetAnimState():GetCurrentAnimationLength(), CheckWarning,
        self)
end
