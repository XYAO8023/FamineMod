...
function FilterBar:RefreshFilterState()
    ...
    for i, filter in ipairs(self.filter_btns) do
        -- local state = Profile:GetCustomizationFilterState(self.filter_category, filter.btnid)--#FIXME
        -- filter.widget:SetFilterState(state)
    end

    if self.sort_btn then
        -- local sort_mode = Profile:GetItemSortMode() or "SORT_RELEASE"--#FIXME
        local sort_mode = "SORT_RELEASE"
...
function FilterBar:AddFilter(text_fmt, on_tex, off_tex, id, filterfn)
    ...
    btn.SetFilterState = function(_, should_enable)
        if should_enable then
            btn:SetHoverText(subfmt(text_fmt, {
                mode = STRINGS.UI.WARDROBESCREEN.FILTER_ON
            }))
            btn.icon:SetTexture("images/button_icons.xml", on_tex)
            self.filters[id] = filterfn
        else
            btn:SetHoverText(subfmt(text_fmt, {
                mode = STRINGS.UI.WARDROBESCREEN.FILTER_OFF
            }))
            btn.icon:SetTexture("images/button_icons.xml", off_tex)
            self.filters[id] = nil
        end
        if not self.no_refresh_picker then self.picker:RefreshItems(self:_ConstructFilter()) end
    end

    local function onclick()
        local was_off = self.filters[id] == nil
        btn:SetFilterState(was_off)
        -- Profile:SetCustomizationFilterState(self.filter_category, id, was_off)--#FIXME
    end
    btn:SetOnClick(onclick)

    table.insert(self.filter_btns, {
        btnid = id,
        widget = btn
    })
...
function FilterBar:AddSorter()
    ...
    btn.SetSortType = function(_, sort_mode)
        ...
        btn:SetHoverText(subfmt(STRINGS.UI.WARDROBESCREEN.SORT_MODE_FMT, {
            mode = STRINGS.UI.WARDROBESCREEN[sort_mode]
        }))
    ...
    local function onclick()
        ...
        if sort_mode == nil then sort_mode = "SORT_RELEASE" end
...
function DamLevDist(a, b, limit)
    ...
    if math.abs(a_len - b_len) > limit then return math.abs(a_len - b_len) end
    ...
    local id = function(i, j) return i * num_columns + j end

    -- Initialize insertion and deletion costs
    for i = 0, a_len do d[id(i, 0)] = i end
    for j = 0, b_len do d[id(0, j)] = j end
    ...
    for i = 1, a_len do
        ...
        for j = 1, b_len do
            ...
            local current = math.min(d[id(i - 1, j)] + 1, -- Deletion
        ...
        if low > limit then return low end
    end
...
local string_search_subwords = function(search, str, sub_len, limit)
    ...
    for i = 1, str_len - sub_len + 1 do
        ...
        if dist <= limit then return true end
    end
...
function FilterBar:AddSearch(thin)
    ...
    local box_size = 145
    if self.thin_mode then box_size = 120 end
    local box_height = 40
    ...
    -- searchbox.textbox:SetForceEdit(true)--#FIXME
    --[[
    ...
    searchbox.textbox.OnTextEntered = function()
        ...
        if not self.no_refresh_picker then self.picker:RefreshItems(self:_ConstructFilter()) end
        if IsNotConsole() then searchbox.textbox:SetEditing(true) end
    ...
    searchbox.textbox.OnTextInputted = function()
        ...
        if self.entered_string ~= searchbox.textbox:GetString() then
            self.search_delay = self.inst:DoTaskInTime(0.25, function() searchbox.textbox:OnTextEntered() end)
        end
    end
    ...
    self.filters["SEARCH"] = function(item_key)
        ...
        if search_match(search_str, string.upper(GetSkinName(item_key))) then return true end
        ...
        local collection_name = nil -- GetItemCollectionName(item_key)--#FIXME
        if collection_name ~= nil then
            if search_match(search_str, string.upper(collection_name)) then return true end
        end
...
function FilterBar:_UpdatePositions()
    ...
    local squeeze = 0
    if self.thin_mode then squeeze = 12 end
    ...
    for a, v in ipairs(self.filter_btns) do
        if v.widget:IsVisible() then
            v.widget:SetPosition(x_offset + -width / 2 + v.widget.size_x / 2 + num_btns * v.widget.size_x - num_btns *
                                     squeeze, 3)
            num_btns = num_btns + 1
    ...
    -- Sort button now
    if self.sort_btn then
        self.sort_btn:SetPosition(x_offset + -width / 2 + self.sort_btn.size_x / 2 + num_btns * self.sort_btn.size_x -
                                      num_btns * squeeze, 3)
        num_btns = num_btns + 1
        if prev_btn then
    ...
    if self.search_box and prev_btn then
        ...
        if self.thin_mode then search_width = 23 end
...
function FilterBar:_ConstructFilter()
    local filter = function(item_key)
        for i, fn in pairs(self.filters) do if not fn(item_key) then return false end end
