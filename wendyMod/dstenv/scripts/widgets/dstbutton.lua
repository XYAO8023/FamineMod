    ...
    if control == self.control and (not self.mouseonly or TheFrontEnd.isprimary) then
        ...
        if down then
            if not self.down then
                ...
                self.down = true
                if self.whiledown then self:StartUpdating() end
...
function Button:OnUpdate(dt) if self.down then if self.whiledown then self.whiledown() end end end
...
function Button:OnGainFocus()
    ...
    if self:IsEnabled() and not self.selected and TheFrontEnd:GetFadeLevel() <= 0 then
        ...
        TheFrontEnd:GetSound():PlaySound("dontstarve/HUD/click_mouseover") -- , nil, ClickMouseoverSoundReduction())--deleted
    end

    if self.ongainfocus then self.ongainfocus(self:IsEnabled()) end
end
...
function Button:OnLoseFocus()
    ...
    if self:IsEnabled() and not self.selected then self.text:SetColour(self.textcolour) end
    self:ResetPreClickPosition()

    self.down = false

    if self.onlosefocus then self.onlosefocus(self:IsEnabled()) end
end
...
function Button:OnUnselect()
    if self:IsEnabled() then
        if self.focus then
            if self.text then
                self.text:SetColour(self.textfocuscolour[1], self.textfocuscolour[2], self.textfocuscolour[3],
                    self.textfocuscolour[4])
            end
        else
            self:OnLoseFocus()
        end
    ...
    if self.onunselect then self.onunselect() end
end

function Button:IsSelected() return self.selected end

function Button:SetOnClick(fn) self.onclick = fn end

function Button:SetOnSelect(fn) self.onselect = fn end

function Button:SetOnUnSelect(fn) self.onunselect = fn end

function Button:SetOnUnselect(fn) self.onunselect = fn end

function Button:SetOnDown(fn) self.ondown = fn end

function Button:SetWhileDown(fn) self.whiledown = fn end

function Button:SetFont(font)
    self.font = font
    if self:IsEnabled() then
        self.text:SetFont(font)
        if self.text_shadow then self.text_shadow:SetFont(font) end
    end
end
...
function Button:SetTextColour(r, g, b, a)
    ...
    if self:IsEnabled() and not self.focus and not self.selected then self.text:SetColour(self.textcolour) end
end
...
function Button:SetTextFocusColour(r, g, b, a)
    ...
    if self.focus and not self.selected then self.text:SetColour(self.textfocuscolour) end
end
...
function Button:SetTextDisabledColour(r, g, b, a)
    ...
    if not self:IsEnabled() then self.text:SetColour(self.textdisabledcolour) end
end
...
function Button:SetTextSelectedColour(r, g, b, a)
    ...
    if self.selected then self.text:SetColour(self.textselectedcolour) end
end
...
function Button:GetText() return self.text:GetString() end
...
function Button:SetText(msg, dropShadow, dropShadowOffset)
    if msg then
        ...
        self.text:Show()
        if self:IsEnabled() then
            self.text:SetColour(self.selected and self.textselectedcolour or
                                    (self.focus and self.textfocuscolour or self.textcolour))
        else
            self.text:SetColour(self.textdisabledcolour)
        end
...
function Button:SetHelpTextMessage(str) if str then self.help_message = str end end

function Button:GetHelpText()
    local controller_id = TheInput:GetControllerID()
    local t = {}
    if (not self:IsSelected() or self.AllowOnControlWhenSelected) and self.help_message ~= "" then
        table.insert(t, TheInput:GetLocalizedControl(controller_id, self.control, false, false) .. " " ..
            self.help_message)
    end
