...
local function getidleanim(inst)
    return inst.components.aura.applying and "attack_loop"
               or (inst.is_defensive and math.random() < 0.1 and "idle_custom") or "idlexxx" -- patched
end

local function startaura(inst)
    if inst.components.health:IsDead() or inst.sg:HasStateTag("dissipate") then return end

    inst.Light:SetColour(255 / 255, 32 / 255, 32 / 255)
    -- deleted--inst.SoundEmitter:PlaySound("dontstarve/characters/wendy/abigail/attack_LP", "angry")
...
local events = {
    ...
    EventHandler("dance", function(inst)
        if not (inst.sg:HasStateTag("dancing") or inst.sg:HasStateTag("busy") or inst.components.health:IsDead()
            or inst.sg:HasStateTag("dissipate")) then inst.sg:GoToState("dance") end
    end)
}
...
local states = {
    State {
        ...
        onenter = function(inst)
            ...
            if anim ~= nil then inst.AnimState:PlayAnimation(anim) end
        end,
    ...
    State {
        ...
        onenter = function(inst)
            ...
            -- note:DS:inst.SoundEmitter:PlaySound("dontstarve/ghost/ghost_girl_howl")
            if inst.components.health ~= nil then inst.components.health:SetInvincible(true) end
        end,
        ...
        onexit = function(inst)
            ...
            if inst._playerlink ~= nil then inst._playerlink.components.ghostlybond:SummonComplete() end
        end
    },
    State {
        name = "dance",
    ...
    State {
        ...
        onenter = function(inst)
            ...
            -- inst.SoundEmitter:PlaySound("dontstarve/characters/wendy/abigail/howl_one_shot")
            -- note:DS:inst.SoundEmitter:PlaySound("dontstarve/ghost/ghost_girl_howl_LP")
    ...
    State {
        ...
        onenter = function(inst)
            inst.Physics:Stop()
            inst.components.aura:Enable(false)
            if inst._playerlink ~= nil then inst._playerlink.components.ghostlybond:RecallComplete() end
            if inst.components.health:IsDead() then inst.components.health:SetCurrentHealth(1) end
        end
    },
    State {
        ...
        timeline = {
            TimeEvent(9 * FRAMES, function(inst)
                -- deleted--inst.SoundEmitter:PlaySound(
                --  inst.sg.statemem.level == 3 and "dontstarve/characters/wendy/abigail/level_change/2" or
                --    "dontstarve/characters/wendy/abigail/level_change/1")
            end),
    ...
    State {
        ...
        events = {
            EventHandler("animover", function(inst)
                if inst.AnimState:AnimDone() then inst.sg:GoToState("walk") end
            end)
        }
    },
    State {
        ...
        timeline = {
            TimeEvent(1 * FRAMES, function(inst)
                if math.random() < 0.1 then
                    -- deleted--inst.SoundEmitter:PlaySound("dontstarve/characters/wendy/abigail/howl")
    ...
    State {
        ...
        events = {
            EventHandler("animover", function(inst)
                if inst.AnimState:AnimDone() then inst.sg:GoToState("run") end
            end)
        }
    },
    State {
        ...
        timeline = {
            TimeEvent(1 * FRAMES, function(inst)
                if math.random() < 0.8 then
                    -- deleted--inst.SoundEmitter:PlaySound("dontstarve/characters/wendy/abigail/howl","howl")
                    inst.SoundEmitter:PlaySound("dontstarve/ghost/ghost_girl_howl", "howl")
