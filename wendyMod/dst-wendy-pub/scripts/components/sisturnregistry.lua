    ...
    local function UpdateSisturnState()
        if POPULATING then
            if self.init_task == nil then
                self.init_task = self.inst:DoTaskInTime(0, function()
                    UpdateSisturnState()
                    self.init_task = nil
                end)
            end
        ...
        if is_active ~= _is_active then
            ...
            TheWorld:PushEvent("onsisturnstatechanged", {
                is_active = is_active
            }) -- Wendy will be listening for this event
        end
    end
    self.UpdateSisturnState = UpdateSisturnState
    --------------------------------------------------------------------------
