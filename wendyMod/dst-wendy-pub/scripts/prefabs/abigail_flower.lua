if not rawget(_G, "TheNet") then return kleiloadlua("scripts/prefabs/abigail_flower.lua")() end
local assets = { -- Asset("ANIM", "anim/abigail_flower.zip"),
    Asset("ANIM", "anim/abigail_flower_rework.zip"),

    Asset("INV_IMAGE", "abigail_flower_level0"),
    Asset("INV_IMAGE", "abigail_flower_level2"),
    Asset("INV_IMAGE", "abigail_flower_level3") -- Asset("INV_IMAGE", "abigail_flower_old"),		-- deprecated, left in for mods
    ...
    ,
    Asset("MINIMAP_IMAGE", "abigail_flower") -- added
}
...
local function UpdateGroundAnimation(inst)
    ...
    if not POPULATING then
        for i, v in ipairs(AllPlayers) do
            if v:HasTag("ghostlyfriend") and not IsEntityDeadOrGhost(v) and v.components.ghostlybond ~= nil
                and v.entity:IsVisible() and (v.sg == nil or not v.sg:HasStateTag("ghostbuild")) then
                local dist = v:GetDistanceSqToPoint(Point(x, y, z)) -- patched, different interface
    ...
    if inst._bond_level ~= level then
        if inst._bond_level == 0 then
            ...
            --deleted--inst.SoundEmitter:PlaySound("dontstarve/characters/wendy/abigail/haunted_flower_LP", "floating")
        elseif inst._bond_level > 0 and level == 0 then
            ...
            --deleted--inst.SoundEmitter:KillSound("floating")
        else
...
local function getstatus(inst, viewer)
    ...
    if inst.components.inventoryitem and inst.components.inventoryitem.owner then -- patched
        _bondlevel = viewer ~= nil and viewer.components.ghostlybond ~= nil and viewer.components.ghostlybond.bondlevel
    end
    return _bondlevel == 3 and "LEVEL3" or _bondlevel == 2 and "LEVEL2" or _bondlevel == 1 and "LEVEL1" or nil
end
...
local function fn()
    ...
    if MakeInventoryFloatable then
        -- #TODO: add anim
        -- MakeInventoryFloatable(inst, "idle_water", "idle_1")--changed
    ...
    inst.clientside_imageoverrides = {abigail_flower_flower_rework = true}
...
Prefab("abigailsummonfx_mount", MakeSummonFX("wendy_mount_channel_flower", "wendy_channel_flower", true),
    assets_summonfx),
