if not rawget(_G, "TheNet") then return kleiloadlua("scripts/prefabs/wendy.lua")() end
...
local prefabs = {
    "abigail", --  "lavaarena_abigail",--deleted
    "abigailsummonfx",
...
-- simplified
local start_inv = {"abigail_flower"}
--[[
...
-- patched
table.insert(prefabs, start_inv[1])

local function OnBondLevelDirty(inst)
    if inst.HUD ~= nil then
        local bond_level = inst._bondlevel:value()
        for i = 0, 3 do
            if i ~= 1 then inst:SetClientSideInventoryImageOverrideFlag("bondlevel" .. i, i == bond_level) end
        end
        if not inst:HasTag("playerghost") then
            if bond_level > 1 then
                if inst.HUD.wendyflowerover ~= nil then inst.HUD.wendyflowerover:Play(bond_level) end
            end
        end
    end
end
...
local function OnPlayerDeactivated(inst)
    ...
    if not TheWorld.ismastersim then inst:RemoveEventCallback("_bondleveldirty", OnBondLevelDirty) end
end
...
local function OnPlayerActivated(inst)
    if inst == ThePlayer then
        ...
        if not TheWorld.ismastersim then inst:ListenForEvent("_bondleveldirty", OnBondLevelDirty) end
        OnBondLevelDirty(inst)
    end
end

local function RefreshFlowerTooltip(inst)
    if inst == ThePlayer then inst:PushEvent("inventoryitem_updatespecifictooltip", {prefab = "abigail_flower"}) end
end
...
local function OnDespawn(inst)
    ...
    if abigail ~= nil and abigail.sg ~= nil and not abigail.inlimbo then
        if not abigail.sg:HasStateTag("dissipate") then abigail.sg:GoToState("dissipate") end
...
local function ghostlybond_onsummon(inst, ghost)
    if inst.components.sanity ~= nil and inst.migration == nil then inst.components.sanity:DoDelta(TUNING.SANITY_MED) end
end
...
local function ghostlybond_onrecall(inst, ghost, was_killed)
    if inst.migration == nil then
        if inst.components.sanity ~= nil then
            -- patched
            if inst.components.sanity.current then
    ...
    inst.components.ghostlybond.ghost:PlaySound(false) -- added
end
...
local function CustomCombatDamage(inst, target)
    return (target:HasDebuff("abigail_vex_debuff")) and TUNING.ABIGAIL_VEX_GHOSTLYFRIEND_DAMAGE_MOD
               or (target == inst.components.ghostlybond.ghost and target:HasTag("abigail")) and 0 or 1
end

-------------------------------------------------------------------------------
local function OnSave(inst, data)
    if inst.questghost ~= nil then data.questghost = inst.questghost:GetSaveRecord() end
end
        ...
        if data.questghost ~= nil and inst.questghost == nil then
            inst:DoTaskInTime(0, function()
                ...
                for k, v in pairs(inst.components.leader.followers) do
                    if k.prefab == data.prefab then k:Remove() end
                end
                local questghost = SpawnSaveRecord(data)
                if inst.migrationpets ~= nil then table.insert(inst.migrationpets, questghost) end
...
local function master_postinit(inst)
    ...
    -- patched
    if not inst.components.foodaffinity then inst:AddComponent("foodaffinity") end
    ...
    elseif TheNet:GetServerGameMode() == "quagmire" then
        ...
        if inst.components.combat.AddDamageModifier then
            inst.components.combat:AddDamageModifier("wendy", TUNING.WENDY_DAMAGE_MULT - 1) -- due to the infamous add method
        else
            inst.components.combat.damagemultiplier = (inst.components.combat.damagemultiplier or 1)
                                                          * TUNING.WENDY_DAMAGE_MULT
        end
...
return MakePlayerCharacter("wendy", prefabs, assets, postinit, start_inv)
