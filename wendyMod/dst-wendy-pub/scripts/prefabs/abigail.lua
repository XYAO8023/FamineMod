-- protect
if not rawget(_G, "TheNet") then return kleiloadlua("scripts/prefabs/abigail.lua")() end
-- added
require "stategraphs/SGabigail"
-- try to fix the anim file bug by changing the bank and re-introducing the old files. But I believe the former is the solution.
local assets = {Asset("SOUND", "sound/ghost.fsb"), Asset("ANIM", "anim/ghost_abigail_old.zip")}
local prefabs = {
    "abigail_attack_fx",
...
local function PlaySound(inst, play)
    if play then
...
local function UpdateGhostlyBondLevel(inst, level)
    local max_health = level == 3 and TUNING.ABIGAIL_HEALTH_LEVEL3 or level == 2 and TUNING.ABIGAIL_HEALTH_LEVEL2
...
-- put them to TUNING
TUNING.ABIGAIL_COMBAT_MUSTHAVE_TAGS = {"combat"} -- changed
-- in order to save polly rogers
TUNING.ABIGAIL_COMBAT_CANTHAVE_TAGS = {
    "INLIMBO",
    "noauradamage",
    "companion" -- added,#TODO:investigate
    ,
    "FX",
    "NOCLICK"
}

TUNING.ABIGAIL_COMBAT_MUSTONEOF_TAGS_AGGRESSIVE = {"monster", "prey", "insect", "hostile", "character", "animal"}
TUNING.ABIGAIL_COMBAT_MUSTONEOF_TAGS_DEFENSIVE = {"monster", "prey"}
...
local function HasFriendlyLeader(inst, target)
    ...
    if leader ~= nil then
        ...
        if target_leader and target_leader.components.inventoryitem then
            ...
            if target_leader == nil then return true end
        end

        local PVP_enabled = TheNet:GetPVPEnabled()

        return leader == target
                   or (target_leader ~= nil
                       and (target_leader == leader or (target_leader:HasTag("player") and not PVP_enabled)))
                   or (target.components.domesticatable and target.components.domesticatable:IsDomesticated()
                       and not PVP_enabled)
...
local function CommonRetarget(inst, v)
    return v ~= inst and v ~= inst._playerlink and v.entity:IsVisible() and v:GetDistanceSqToInst(inst._playerlink)
               < COMBAT_TARGET_DSQ and inst.components.combat:CanTarget(v) and v.components.minigame_participator == nil
...
local function DefensiveRetarget(inst)
    ...
    elseif not IsWithinDefensiveRange(inst) then
        ...
        local entities_near_me = TheSim:FindEntities(ix, iy, iz, TUNING.ABIGAIL_DEFENSIVE_MAX_FOLLOW,
            TUNING.ABIGAIL_COMBAT_MUSTHAVE_TAGS, TUNING.ABIGAIL_COMBAT_CANTHAVE_TAGS,
            TUNING.ABIGAIL_COMBAT_MUSTONEOF_TAGS_DEFENSIVE)
        local leader = inst.components.follower.leader

        for _, v in ipairs(entities_near_me) do
            if CommonRetarget(inst, v)
                and (v.components.combat.target == inst._playerlink or inst._playerlink.components.combat.target == v
                    or v.components.combat.target == inst) then
                -- insert logic for ice staff and panflute
                if v.sg then
                    if v.sg:HasStateTag("sleeping") or v.sg:HasStateTag("frozen") then
                    else
                        return v
                    end
                else
...
local function AggressiveRetarget(inst)
    if inst._playerlink == nil then
        ...
        local ix, iy, iz = inst.Transform:GetWorldPosition()
        local entities_near_me = TheSim:FindEntities(ix, iy, iz, TUNING.ABIGAIL_COMBAT_TARGET_DISTANCE,
            TUNING.ABIGAIL_COMBAT_MUSTHAVE_TAGS, TUNING.ABIGAIL_COMBAT_CANTHAVE_TAGS,
            TUNING.ABIGAIL_COMBAT_MUSTONEOF_TAGS_AGGRESSIVE)

        local leader = inst.components.follower.leader
        for _, v in ipairs(entities_near_me) do if CommonRetarget(inst, v) then return v end end
...
local function StartForceField(inst)
    if not inst.sg:HasStateTag("dissipate") and not inst:HasDebuff("forcefield")
        and (inst.components.health == nil or not inst.components.health:IsDead()) then
...
local function OnAttacked(inst, data)
    ...
    elseif not data.attacker:HasTag("noauradamage") then
        ...
        elseif inst:IsWithinDefensiveRange() and inst._playerlink:GetDistanceSqToInst(data.attacker)
            < ABIGAIL_DEFENSIVE_MAX_FOLLOW_DSQ then
    ...
    if inst:HasDebuff("forcefield") then
        if data.attacker ~= nil and data.attacker ~= inst._playerlink and data.attacker.components.combat ~= nil then
            ...
            if elixir_buff ~= nil and elixir_buff.prefab == "ghostlyelixir_retaliation_buff" then
                ...
                -- deleted--inst.SoundEmitter:PlaySound("dontstarve/characters/wendy/abigail/shield/on")
            else
                -- deleted--inst.SoundEmitter:PlaySound("dontstarve/characters/wendy/abigail/shield/on")
            end
        end
...
local function OnDeath(inst)
    ...
    -- patched
    if not inst._playerlink then inst:DoTaskInTime(0, inst.Remove) end
    inst:PlaySound(false) -- added
end
...
local function auratest(inst, target)
    if target == inst._playerlink then return false end

    if target.components.minigame_participator ~= nil then return false end

    if (target:HasTag("player") and not TheNet:GetPVPEnabled()) or target:HasTag("ghost")
        or target:HasTag("noauradamage") then return false end

    local leader = inst.components.follower.leader
    if leader ~= nil
        and (leader == target or (target.components.follower ~= nil and target.components.follower.leader == leader)) then
        return false
    end

    if inst.is_defensive and not IsWithinDefensiveRange(inst) then return false end

    -- insert logic for ice staff and panflute
    if target.sg then
        if target.sg:HasStateTag("sleeping") or target.sg:HasStateTag("frozen") then
            if inst.is_defensive then return false end
        end
    end
    ...
    if target.components.combat.target ~= nil
        and (target.components.combat.target == inst or target.components.combat.target == leader) then return true end

    local ismonster = target:HasTag("monster")
    if ismonster and not TheNet:GetPVPEnabled()
        and ((target.components.follower and target.components.follower.leader ~= nil
            and target.components.follower.leader:HasTag("player")) or target.bedazzled) then return false end
    -- new: save polly rogers
    return ismonster or target:HasTag("prey") and not target:HasTag("companion")
end
...
local function UpdateDamage(inst)
    ...
    if TheWorld.state.inst and TheWorld.state.inst:IsCave() then phase = "night" end
    inst.components.combat.defaultdamage = (TUNING.ABIGAIL_DAMAGE[phase] or TUNING.ABIGAIL_DAMAGE.day)
                                               / TUNING.ABIGAIL_VEX_DAMAGE_MOD -- so abigail does her intended damage defined in tunings.lua

    inst.attack_level = phase == "day" and 1 or phase == "dusk" and 2 or 3
    ...
    if inst.attack_fx_ground
        and not inst.attack_fx_ground.AnimState:IsCurrentAnimation("attack" .. level_str .. "_ground_loop") then
...
local function AbigailHealthDelta(inst, data)
    local MYMODNAME = "ADORABLEABBY"
    local tuning = TUNING[MYMODNAME]
    if inst._playerlink ~= nil then
        ...
        elseif data.oldpercent < data.newpercent and data.newpercent > 0.33 then
            if inst.issued_health_warning then inst.issued_health_warning = false end
        end
    end
end
...
local function on_ghostlybond_level_change(inst, player, data)
    if not inst.inlimbo and data.level > 1 and not inst.sg:HasStateTag("busy")
        and (inst.components.health == nil or not inst.components.health:IsDead()) then
...
local function BecomeDefensive(inst)
    ...
    inst:RemoveTag("aggressive")
    if inst._playerlink ~= nil then inst._playerlink:RemoveTag("has_aggressive_follower") end
...
local function linktoplayer(inst, player)
    ...
    if player.components.pethealthbar ~= nil then
        ...
        local elixir_buff = inst:GetDebuff("elixir_buff")
        if elixir_buff then player.components.pethealthbar:SetSymbol(elixir_buff.prefab) end
    end
...
local function OnExitLimbo(inst)
    local level = (inst._playerlink ~= nil and inst._playerlink.components.ghostlybond ~= nil)
                      and inst._playerlink.components.ghostlybond.bondlevel or 1
    local light_vals = TUNING.ABIGAIL_LIGHTING[level] or TUNING.ABIGAIL_LIGHTING[1]
...
local function getstatus(inst)
    ...
    return bondlevel == 3 and "LEVEL3" or bondlevel == 2 and "LEVEL2" or "LEVEL1"
end

local function onsave(inst, data)
    if inst._playerlink then
        if inst._playerlink.components.ghostlybond then
            inst._playerlink.components.ghostlybond.ghost_prefab = "abigail"
            inst._playerlink.components.ghostlybond.ghostinlimbo = inst:HasTag("INLIMBO")
        end
    end
end
local function fn()
    ...
    -- bank=abigail, anim=idlexxx
    inst.AnimState:SetBank("abigail")
    inst.AnimState:SetBuild("ghost_abigail_old")
    inst.AnimState:PlayAnimation("idlexxx", true)
    -- inst.AnimState:SetBloomEffectHandle("shaders/anim_bloom_ghost.ksh")--deleted,failed
    ...
    inst:AddTag("amphibious") -- added
    local make = MakeAmphibiousGhostPhysics or MakeGhostPhysics
    make(inst, 1, .5) -- changed
    ...
    inst.PlaySound = PlaySound -- added
    inst:PlaySound(true) -- uncommented
    ...
    inst.components.health.fire_damage_scale = 0 -- added, no fire damage
    inst.components.health.vulnerabletoheatdamage = false -- added, no fire damage
    ...
    if inst.components.follower.SetFollowExitDestinations then
        inst.components.follower:SetFollowExitDestinations({EXIT_DESTINATION.LAND, EXIT_DESTINATION.WATER}) -- added
    end
    ...
    inst._on_ghostlybond_level_change = function(player, data)
...
local function SetRetaliationTarget(inst, target)
    ...
    if s ~= 1 and s ~= 0 then inst.Transform:SetScale(s, s, s) end
...
local function DoRetaliationDamage(inst)
    ...
    if target ~= nil and target:IsValid() and not target.inlimbo and target.components.combat ~= nil then
        ...
        -- deleted--inst.SoundEmitter:PlaySound("dontstarve/characters/wendy/abigail/shield/retaliation_fx")
...
local function retaliationattack_fn()
    ...
    -- inst.AnimState:SetBloomEffectHandle("shaders/anim_bloom_ghost.ksh")--deleted
    inst.AnimState:SetBloomEffectHandle("shaders/anim.ksh") -- added
...
local function on_target_attacked(inst, target, data)
    if data ~= nil and data.attacker ~= nil and data.attacker:HasTag("ghostlyfriend") then inst.hitevent:push() end
end

local function buff_OnExtended(inst)
    if inst.decaytimer ~= nil then inst.decaytimer:Cancel() end
    inst.decaytimer = inst:DoTaskInTime(TUNING.ABIGAIL_VEX_DURATION, function()
        inst.components.debuff:Stop()
    end)
end

local function buff_OnAttached(inst, target)
    if target ~= nil and target:IsValid() and not target.inlimbo and target.components.combat ~= nil
...
local function abigail_vex_debuff_fn()
    ...
    if not TheNet:IsDedicated() then inst:ListenForEvent("abigail_vex_debuff.hitevent", do_hit_fx) end
    ...
    inst.persists = false
    inst._on_target_attacked = function(target, data)
        on_target_attacked(inst, target, data)
    end
...
return Prefab("abigail", fn, assets, prefabs),
    Prefab("abigail_retaliation", retaliationattack_fn, {Asset("ANIM", "anim/abigail_shield.zip")}), Prefab(
        "abigail_vex_debuff", abigail_vex_debuff_fn, {Asset("ANIM", "anim/abigail_debuff_fx.zip")}, {"abigail_vex_hit"}),
