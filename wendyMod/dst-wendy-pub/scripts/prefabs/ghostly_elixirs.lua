local potion_tunings = {
    ...
    ghostlyelixir_attack = {
        ONAPPLY = function(inst, target) if target.UpdateDamage ~= nil then target:UpdateDamage() end end,
        ONDETACH = function(inst, target)
            if target:IsValid() and target.UpdateDamage ~= nil then target:UpdateDamage() end
        end,
...
local function DoApplyElixir(inst, giver, target)
    return target:AddDebuff("elixir_buff", inst.buff_prefab, nil, nil, function()
        ...
        if cur_buff ~= nil and cur_buff.prefab ~= inst.buff_prefab then target:RemoveDebuff("elixir_buff") end
    end)
end
...
local function potion_fn(anim, potion_tunings, buff_prefab)
    ...
    if potion_tunings.FLOATER ~= nil then
        -- MakeInventoryFloatable(inst, potion_tunings.FLOATER[1], potion_tunings.FLOATER[2], potion_tunings.FLOATER[3])--deleted temporary
    else
        -- MakeInventoryFloatable(inst)--deleted temporary
    end
    if MakeInventoryFloatable then inst:AddComponent("floatable") end
...
local function buff_OnAttached(inst, target)
    ...
    if inst.potion_tunings.ONAPPLY ~= nil then inst.potion_tunings.ONAPPLY(inst, target) end
...
local function buff_OnTimerDone(inst, data) if data.name == "decay" then inst.components.debuff:Stop() end end
...
local function buff_OnDetached(inst, target)
    ...
    if inst.potion_tunings.ONDETACH ~= nil then inst.potion_tunings.ONDETACH(inst, target) end
    inst:Remove()
end
...
local function buff_fn(tunings, dodelta_fn)
    ...
    inst:AddTag("INTERIOR_LIMBO_IMMUNE") -- bug fix: entering interior areas removes non-followers
...
local function AddPotion(potions, name, anim)
    ...
    local assets = {Asset("ANIM", "anim/ghostly_elixirs.zip"), Asset("ANIM", "anim/abigail_buff_drip.zip")}
    local prefabs = {buff_prefab, potion_tunings[potion_prefab].fx, potion_tunings[potion_prefab].dripfx}
