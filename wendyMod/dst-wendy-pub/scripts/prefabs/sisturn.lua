...
local assets = {Asset("ANIM", "anim/sisturn.zip"), Asset("ANIM", "anim/ui_chest_2x2.zip")} -- Asset("MINIMAP_IMAGE", "sisturn")}--added
...
local function onhammered(inst)
    ...
    -- fx:SetMaterial("wood")--deleted
    inst:Remove()
end

local function onhit(inst, worker, workleft)
    if workleft > 0 and not inst:HasTag("burnt") then
        -- deleted--inst.SoundEmitter:PlaySound("dontstarve/characters/wendy/sisturn/hit")
...
local function onbuilt(inst)
    ...
    -- deleted--inst.SoundEmitter:PlaySound("dontstarve/characters/wendy/sisturn/place")
...
local function update_idle_anim(inst)
    ...
    if IsFullOfFlowers(inst) then
        ...
        -- deleted--inst.SoundEmitter:PlaySound("dontstarve/characters/wendy/sisturn/LP", "sisturn_on")
    else
        inst.AnimState:PlayAnimation("on_pst")
        inst.AnimState:PushAnimation("idle", false)
        -- deleted--inst.SoundEmitter:KillSound("sisturn_on")
    end
end
...
local function AddDecor(inst, data)
    ...
    local f = data.item
    if f then
        if f.prefab == "petals" then
            local p = f.components.perishable
            if p then
                -- added
                local oldp = p.perishfn
                local function perishfn(inst)
                    local owner = inst.components.inventoryitem and inst.components.inventoryitem.owner
                    if owner then
                        local holder = owner.components.container
                        if holder then
                            if owner.prefab == "sisturn" then
                                p.onperishreplacement = "ghostflower"
                            end
                        end
                    end
                end
...
local function getstatus(inst)
    ...
    return num_decor >= num_slots and "LOTS_OF_FLOWERS" or num_decor > 0 and "SOME_FLOWERS" or nil
end
...
local function OnLoad(inst, data)
    if data ~= nil and data.burnt and inst.components.burnable ~= nil then inst.components.burnable.onburnt(inst) end
end
local function fn()
    ...
    inst:AddTag("pogproof") -- added, because pogs always want to but cannot reach sisturn
    ...
    for _, v in ipairs(FLOWER_LAYERS) do inst.AnimState:Hide(v) end
    ...
    -- MakeHauntableWork(inst)--deleted
    ...
    if TheWorld.components.sisturnregistry == nil then TheWorld:AddComponent("sisturnregistry") end
...
return Prefab("sisturn", fn, assets, prefabs), MakePlacer("sisturn_placer", "sisturn", "sisturn", "placer")
