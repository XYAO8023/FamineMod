local assets = {Asset("ANIM", "anim/abigail_shield.zip")}

local function buff_OnAttached(inst, target)
    target:AddTag("forcefield")

    if target.components.health ~= nil then
        target.components.health.externalabsorbmodifiers:SetModifier(inst, TUNING.ABIGAIL_FORCEFIELD_ABSORPTION,
            "forcefield")
    end
...
return MakeBuffFx("abigailforcefield", "shield"), MakeBuffFx("abigailforcefieldbuffed", "shield_buff"),
