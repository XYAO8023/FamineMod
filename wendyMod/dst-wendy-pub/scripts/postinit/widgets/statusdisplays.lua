...
local MoistureMeter = Widget -- require "widgets/hungerbadge"
local WereBadge = Widget -- require "widgets/hungerbadge"
local BoatMeter = Widget -- require "widgets/boatmeter"
local PetHealthBadge = require "widgets/pethealthbadge"
local InspirationBadge = Widget -- require "widgets/inspirationbadge"
local MightyBadge = Widget -- require "widgets/mightybadge"
local ResurrectButton = Widget -- require "widgets/resurrectbutton"
...
local function OnSetPlayerMode(inst, self)
    ...
    if self.onhealthdelta == nil then
        self.onhealthdelta = function(owner, data)
            self:HealthDelta(data)
        end
    ...
    if self.onhungerdelta == nil then
        self.onhungerdelta = function(owner, data)
            self:HungerDelta(data)
        end
        self.inst:ListenForEvent("hungerdelta", self.onhungerdelta, self.owner)
        self:SetHungerPercent(self.owner.components.hunger:GetPercent())
    end

    if self.onsanitydelta == nil then
        self.onsanitydelta = function(owner, data)
            self:SanityDelta(data)
        end
        self.inst:ListenForEvent("sanitydelta", self.onsanitydelta, self.owner)
        self:SetSanityPercent(self.owner.components.sanity:GetPercent())
    end

    if self.onmoisturedelta == nil then
        self.onmoisturedelta = function(owner, data)
            self:MoistureDelta(data)
        end
    ...
    if self.ongotonplatform == nil then
        ...
        self.ongotonplatform = function(owner, platform)
            if platform.components.healthsyncer ~= nil then
                self.boatmeter:Enable(platform)
            end
        end
        self.inst:ListenForEvent("got_on_platform", self.ongotonplatform, self.owner)

        self.ongotoffplatform = function(owner, platform)
            self.boatmeter:Disable()
        end
    ...
    if self.wereness ~= nil and self.onwerenessdelta == nil then
        self.onwerenessdelta = function(owner, data)
            self:WerenessDelta(data)
        end
    ...
    if self.inspirationbadge ~= nil and self.oninspirationdelta == nil then
        self.oninspirationdelta = function(owner, data)
            self:SetInspiration(data ~= nil and data.newpercent or 0, data ~= nil and data.slots_available or nil,
                data ~= nil and data.draining)
        end
        self.inst:ListenForEvent("inspirationdelta", self.oninspirationdelta, self.owner)

        self.oninspirationsongchanged = function(owner, data)
            self:OnInspirationSongChanged(data ~= nil and data.slotnum or 0,
                data ~= nil and data.songdata ~= nil and data.songdata.NAME or nil)
        end
    ...
    if self.pethealthbadge ~= nil and self.onpethealthdirty == nil then
        self.onpethealthdirty = function()
...
local initfn = function(self, owner)
    ...
    self.brain = self.brain
                     or self:AddChild(
            owner.CreateSanityBadge ~= nil and owner.CreateSanityBadge(owner) or SanityBadge(owner))
    self.onsanitydelta = nil

    self.stomach = self.stomach
                       or self:AddChild(
            owner.CreateHungerBadge ~= nil and owner.CreateHungerBadge(owner) or HungerBadge(owner))
    self.onhungerdelta = nil

    self.heart = self.heart
                     or self:AddChild(
            owner.CreateHealthBadge ~= nil and owner.CreateHealthBadge(owner) or HealthBadge(owner))
    -- self.heart.effigybreaksound = "dontstarve/creatures/together/lavae/egg_deathcrack"
    self.onhealthdelta = nil
    self.healthpenalty = 0
    --[[

    self.moisturemeter = self.moisturemeter or self:AddChild(MoistureMeter(owner))
    --deleted something
    self.onmoisturedelta = nil

    self.boatmeter = self.boatmeter or self:AddChild(BoatMeter(owner))
    self.boatmeter:SetPosition(-80, -40, 0)
    self.ongotonplatform = nil
    self.ongotoffplatform = nil
    self.resurrectbutton = self.resurrectbutton or self:AddChild(ResurrectButton(owner))
    ...
    self.resurrectbuttonfx.inst:ListenForEvent("animover", function(inst)
        inst.widget:Hide()
    end)
    ...
    self.inst:ListenForEvent("gotnewattunement", function(owner, data)
        if data.proxy:IsAttunableType("remoteresurrector") then
            ...
            self.rezbuttontask = not self.heart.effigy and
                                     self.inst:DoTaskInTime(rezbuttondelay, UpdateRezButton, self, true) or nil
        end
    end, owner)
    ...
    self.inst:ListenForEvent("attunementlost", function(owner, data)
        ...
        if data.proxy:IsAttunableType("remoteresurrector") and
            ...
            self.rezbuttontask = self.heart.effigy and
                                     self.inst:DoTaskInTime(rezbuttondelay, UpdateRezButton, self, false) or nil
        end
    end, owner)
]]
    self.rezbuttontask = nil
    ...
    if owner.components.pethealthbar ~= nil then
        if owner.prefab == "wendy" then
            self.pethealthbadge = self:AddChild(PetHealthBadge(owner, {254 / 255, 253 / 255, 237 / 255, 1},
                "status_abigail"))
            -- patched temporary
            local COMBINEDSTATUS = TUNING.combined_status == "true" or KnownModIndex:IsModEnabled("workshop-574636989")
            if COMBINEDSTATUS then
                local METHOD = TUNING.combined_status_method
                if METHOD == 1 then
                    -- one clock
                    self.pethealthbadge:SetPosition(-63, -60, 0)
                else
                    -- two clocks
                    self.pethealthbadge:SetPosition(-63, -52, 0)
                end
            else
                self.pethealthbadge:SetPosition(40, -100, 0)
            end
            if self.moisturemeter then self.moisturemeter:SetPosition(-40, -100, 0) end
        end
    end
...
function StatusDisplays:ShowStatusNumbers()
    ...
    self.heart.num:Show()
    if self.moisturemeter then self.moisturemeter.num:Show() end
    if self.boatmeter and self.boatmeter.boat then self.boatmeter.num:Show() end
    if self.pethealthbadge then self.pethealthbadge.num:Show() end
    if self.wereness ~= nil then self.wereness.num:Show() end
end
...
function StatusDisplays:HideStatusNumbers()
    ...
    self.heart.num:Hide()
    if self.moisturemeter then self.moisturemeter.num:Hide() end
    if self.boatmeter then self.boatmeter.num:Hide() end
    if self.pethealthbadge then self.pethealthbadge.num:Hide() end
    if self.wereness ~= nil then self.wereness.num:Hide() end
end
...
function StatusDisplays:SetWereMode(weremode, nofx)
    ...
    elseif weremode then
        ...
        self.wereness:Show()
        self.wereness:SetPosition(self.stomach:GetPosition())
        if not nofx then self.wereness:SpawnNewFX() end
    else
        self.stomach:Show()
        self.wereness:Hide()
        if not nofx then self.wereness:SpawnShatterFX() end
    end
end
...
function StatusDisplays:SetGhostMode(ghostmode)
    ...
    if self.pethealthbadge ~= nil then self.pethealthbadge:Show() end
    if self.inspirationbadge ~= nil then self.inspirationbadge:Show() end

    if self.mightybadge ~= nil then self.mightybadge:Show() end
    -- end
    if self.rezbuttontask ~= nil then
        self.rezbuttontask:Cancel()
        self.rezbuttontask = nil
    end
    -- self:EnableResurrect(self.owner.components.attuner ~= nil and
    --                       self.owner.components.attuner:HasAttunement("remoteresurrector"))

    if self.modetask ~= nil then self.modetask:Cancel() end
...
function StatusDisplays:SetHungerPercent(pct)
    self.stomach:SetPercent(pct, self.owner.components.hunger:Max())
...
function StatusDisplays:SetSanityPercent(pct)
    self.brain:SetPercent(pct, self.owner.components.sanity:Max(), self.owner.components.sanity:GetPenaltyPercent())

    if self.owner.components.sanity:IsInsane() or self.owner.components.sanity:IsEnlightened() then
...
function StatusDisplays:RefreshPetHealth()
    ...
    self.pethealthbadge:SetValues(pethealthbar:GetSymbol(), pethealthbar:GetPercent(), pethealthbar:GetOverTime(),
        pethealthbar:GetMaxHealth(), pethealthbar:GetPulse())
...
return function(self, ...)
    for k, v in pairs(StatusDisplays) do if not self[k] then self[k] = v end end
    initfn(self, ...)
end
