...
function DoRPCAction(action, item, target, actiontype, doer, pos, ent, id)
    ...
    local rpc = nil
    local distance = act == ACTIONS.CASTAOE and target ~= nil and target.components.aoetargeting ~= nil
                         and target.components.aoetargeting:GetRange() or nil
    ...
    if actiontype == "CLICK" then
        pos = pos or (target and thescreen.xyz2v(thescreen.getworld(target)))
                  or (item and thescreen.xyz2v(thescreen.getworld(item)))
    end
    ...
    if not buffact:is_a(BufferedAction) then
        buffact = BufferedAction(doer, target, act, item, pos, nil, distance, nil, nil)
    else
        -- this is actually a buffact
        act = buffact.action or act.action or act
        buffact.pos = buffact.pos or pos ~= nil and DynamicPosition(pos)
    end
    ...
    if IsClient() then
        local function cb()
            if rpc == RPC.RightClick then
                SendRPCToServer(rpc, act.code, buffact.pos.local_pt.x, buffact.pos.local_pt.z, target, rotation,
                    ismousereleased, cmode, canforce, buffact.action.mod_name, buffact.pos.walkable_platform,
            ...
            elseif rpc == RPC.LeftClick then
                ...
                SendRPCToServer(rpc, act.code, buffact.pos.local_pt.x, buffact.pos.local_pt.z, target, ismousereleased,
                    cmode, canforce, buffact.action.mod_name, buffact.pos.walkable_platform,
                    buffact.pos.walkable_platform ~= nil, ent, id)
            else
                SendRPCToServer(rpc, act.code, item, target)
            end
        end
...
local function ClientUseItemFromInv(inv, actiontype, item, target)
    ...
    if actiontype == "SCENE" then
        ...
        elseif item.replica.equippable ~= nil and item.replica.equippable:IsEquipped() then
...
function UseItemFromInventory(actiontype, item, target)
    ...
    if type(actiontype) ~= "string" then
        CONSOLE.err("UseItemFromInventory: type is not string", "type is", actiontype)
        return false
    end
    if not item then
...
function DoAttack(target, forced, guy)
    ...
    if target == nil then
        ...
        if not self.ismastersim and self.locomotor == nil and self.remote_controls[CONTROL_ATTACK] == nil then
...
function MatchCriteria(item, criteria, inst)
    ...
    if criteria.must then for i, v in ipairs(criteria.must) do if not item:HasTag(v) then return false end end end
    if criteria.cant then if item:HasOneOfTags(criteria.cant) then return false end end
...
function GetSelectedItemsFromInventory(inv, criteria)
    ...
    MapDict(invs, function(_, pack)
        if pack then
            ...
            MapDict(selecteditems, function(__, item)
                mapping[item.GUID or math.random()] = item
            end)
        end
    end)
    MapDict(mapping, function(_, item)
...
function PlayerTalk(str, ...)
    PlayerTalkText(FormatString(str, ...))
end
...
function LocalSay(name, text, color_param, icons)
    ...
    local function getname(inst)
        return inst and (inst.name or inst.GetDisplayName and inst:GetDisplayName()) or "???"
    end

    if not name then name = getname(ThePlayer) end
    local guid, userid, name, prefab, message, colour, whisper, isemote, user_vanity = -1, -1, name, nil, text, color,
        false, false, type(icons) == "string" and {icons} or icons
...
function GetActiveScreen()
    if safefetch(GLOBAL, "TheFrontEnd", "GetActiveScreen") then
        return safefetch(TheFrontEnd:GetActiveScreen(), "name")
    else
        return ""
    end
end
...
function IsNotGameScreen()
    ...
    if not ThePlayer.HUD then return false end
    local h = ThePlayer.HUD
    local tests = {
        GetActiveScreen():find("HUD") == nil,
        not not h.writeablescreen,
        h.IsCraftingOpen and h:IsCraftingOpen(),
        h.IsGroomerScreenOpen and h:IsGroomerScreenOpen(),
        h.IsPlayerAvatarPopUpOpen and h:IsPlayerAvatarPopUpOpen()
    }
    for k, v in pairs(tests) do if v then return true end end
    return false
end
    ...
    if not self.draghandlers then
        ...
        local new = function(inst, button, down, x, y)
            ...
            if down then
                ...
                for i, v in ipairs(self.draghandlers.start) do v(inst, button, down, x, y, 0) end
            else
                self.dragging = false
                for i, v in ipairs(self.draghandlers.stop) do v(inst, button, down, x, y, 2) end
        ...
        if TheInput:IsControlPressed(CONTROL_ZOOM_IN) or TheInput:IsControlPressed(CONTROL_ZOOM_OUT) then
            ...
            if element then
                if self.inst.HUD ~= nil then
                    if not element.parent then element = element.widget or element end
                    local maxdepth = 100
...
local function HackCameraControl()
    if HasHUD() then
...
function AddWheelHandler(self, onwheel)
    ...
    if not RegisterMod("CameraZoomBlock") then HackCameraControl() end
end
...
theinput = {
    ...
    getmouse = function()
        ...
        local all = self.entitiesundermouse or TheSim:GetEntitiesAtScreenPoint(TheSim:GetPosition()) or {}
    ...
    getundermouse = function()
        local self = TheInput
        local hover = self.hoverinst or self.entitiesundermouse and self.entitiesundermouse[1]
        if hover and hover.entity:IsValid() and hover.entity:IsVisible() then
        else
            hover = nil
        end
...
thescreen = {
    ...
    isValid = function(x, y, z)
        ...
        if type(x) == "table" then return x.x and (x.x ~= 0 or x.y ~= 0 or (x.z and x.z ~= 0)) end
        return x and (x ~= 0 or y ~= 0 or (z and z ~= 0))
    end,
    canConvert = function(x, y, z)
        return type(x) == "number" and type(y) == "number" and (not z or type(z) == "number")
    end,
    -- input=nil
    -- output=minimap
    hasMap = function()
    ...
    getscreen = function(widget)
        ...
        if widget.AnimState then
            x, y = TheSim:GetScreenPos(widget.AnimState:GetSymbolPosition("", 0, 0, 0))
            valid = true
        end
    ...
    map2world = function(x, y)
        ...
        if thescreen.hasMap() then return thescreen.minimap:MapPosToWorldPos(x, y, 0) end
        return 0, 0, 0
    end,
    -- input=x,y,z
    -- output=x,y
    world2map = function(x, y, z)
        ...
        if thescreen.hasMap() then return thescreen.minimap:WorldPosToMapPos(x, z, 0) end
        return 0, 0
    end,
    mouse2world = function()
        return thescreen.screen2world(thescreen.getmouse())
    end,
    mouse2map = function()
        return thescreen.screen2map(thescreen.getmouse())
    end,
...
function MovementPrediction(enable, toggle)
    ...
    local pc = ThePlayer.components.playercontroller
    local current = toggle and (not Profile:GetMovementPredictionEnabled()) or enable
...
theshard = {
    ...
    _worldeventpusher = function(worldid, ready)
        if TheWorld then
            TheWorld:PushEvent(REMOTESHARDSTATE.READY == ready and "shardconnected" or "sharddisconnected", worldid)
        end
    end,
    _listeners = {},
    _listener = function(...)
    ...
    getshard = function()
        return Shard_GetConnectedShards()
    end,
    ...
    isavailable = function(id)
        return Shard_IsWorldAvailable(tostring(id))
    end,
    isself = function(id)
        return id == theshard.getid()
    end,
    notself = function(id)
        return not theshard.isself(id)
    end,
    listen = function(fn)
    ...
    unlisten = function(fn)
        table.removev(theshard._listeners, fn)
    end,
    createevent = function()
        theshard.listen(theshard._worldeventpusher)
    end
}
